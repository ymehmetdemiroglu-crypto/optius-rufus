...
VPS üzerinde bir Amazon listing optimizasyon SaaS platformu kurmak için teknik olarak **6 ana bileşene** ihtiyacın var: **VPS Hosting** (~$33/ay), **Web Uygulaması** (React + Node.js/Python), **Veritabanı** (PostgreSQL + Vector DB), **AI/Embedding Katmanı** (OpenAI API ~$2–5/ay), **Ödeme Sistemi** (Stripe/Paddle ~%2.9–5% komisyon), ve **Monitoring/Auth** (~$20/ay). Toplam **MVP aylık maliyet ~$100–150/ay** (₺3,500–5,250). Geliştirme süresi **17 hafta (~4 ay)** ve geliştirme maliyeti **~$12,000–20,000** (dışarıdan yazılımcı ile). Eğer kendin yazarsan maliyet sadece **vakit** — teknoloji yığını React + TypeScript + tRPC + Drizzle ORM + Hono + PostgreSQL ile tam olarak senin ihtiyacını karşılıyor. Bu rehber, tüm bileşenleri, maliyetleri, karşılaştırmaları ve adım adım uygulama planını içeriyor.
...
Platformunun çalışma mantığı basit ama güçlü: müşteri web arayüzüne giriş yapar → ASIN girer → sistem Apify / Rainforest API ile listing verilerini çeker → AI embedding katmanı listing'i ve rakip ürünleri vektör uzayına dönüştürür → semantic gap analizi yapar → optimize edilmiş listing önerisi (title, bullets, Q&A, A+ content) sunar → müşteri raporu görüntüler ve kopyalar.
...
| **Veritabanı** | PostgreSQL (Drizzle ORM) + pgvector [^SKILL2^] | $0 (VPS üzerinde) | $0 | Kritik |
...
- **PostgreSQL 16:** Drizzle ORM ile type-safe veritabanı işlemleri [^SKILL2^]
...
services:
  app:
    ...
    ports: ["3000:3000"]
    environment:
      - DATABASE_URL=postgres://postgres:pass@db:5432/amazon_optimizer
...
### 3.2 Backend: Hono + tRPC 11 + Drizzle ORM + PostgreSQL

Backend için **Hono** (hafif ve hızlı web framework) + **tRPC 11** (end-to-end type-safe API) + **Drizzle ORM** (type-safe SQL query builder) + **PostgreSQL** kombinasyonu, skill setimizin doğal bir uzantısı [^SKILL2^]. Bu kombinasyonun en büyük avantajı **type safety** — frontend ve backend arasında paylaşılan tipler sayesinde, API çağrılarında hata yapma olasılığı neredeyse sıfır.
...
export const optimizeRouter = router({
  analyze: publicProcedure
    ...
    .mutation(async ({ input }) => {
      // 1. Apify / Rainforest API'den listing verisini çek
      const listing = await fetchListingFromScraper(input.asin);
      // 2. Embedding oluştur
...
pgvector, mevcut PostgreSQL veritabanına eklenebilir ama 5M+ vektörde performans düşüyor [^111^]. Qdrant, Rust-native implementasyonu ile daha yüksek concurrency ve daha düşük latency sunuyor.

---

## 5. Bileşen 4: Ödeme ve Abonelik Sistemi
...
PostgreSQL + Drizzle ORM kullanarak platformunun veri modeli şu şekilde yapılandırılabilir [^SKILL2^]:
...
export const users = pgTable("users", {
  ...
  subscriptionTier: pgEnum("tier", ["free", "starter", "pro", "enterprise"]).default("free"),
...
export const optimizationReports = pgTable("optimization_reports", {
  id: serial("id").primaryKey(),
  listingId: bigint("listing_id", { mode: "number" }).references(() => listings.id),
...
Embedding vektörleri PostgreSQL'de JSON olarak saklanabilir ama similarity search (cosine similarity) için özel bir vector DB daha performanslı. Qdrant'ta koleksiyon yapısı:
...
- Scraper API rate limit kullanımı
...
- **API Rate Limiting:** Scraper API rate limit'leri (1 request/2 seconds) aşılmamalı
...
- **Data Encryption:** API anahtarları environment variable'da, database'de encrypted
...
MVP (Minimum Viable Product) kapsamında platformun temel özellikleri şunlar olmalı: kullanıcı kayıt/giriş, ASIN ile listing çekme (Apify / Rainforest API), embedding + semantic gap analizi, optimize edilmiş listing önerisi (title + bullets), ve rapor görüntüleme.

| Faz | Süre | Teslimat | Maliyet* |
|-----|------|---------|---------|
| **Faz 1: Tasarım & UX** | 2 hafta | Wireframe, UI mockup, kullanıcı akışları | $800 |
| **Faz 2: Backend API** | 4 hafta | tRPC router'ları, DB schema, auth, scraper entegrasyonu | $3,200 |
...
- PostgreSQL + Qdrant Docker compose up
...
| Scraper API Otomasyon | Sınırlı | Hayır | Hayır | **Tam** |
...
- [ ] PostgreSQL + Qdrant Docker compose kurulumu
- [ ] Apify / Rainforest API hesabı açma
...
**İlk Ay Sonunda:**
- [ ] ASIN ile listing çekme (Apify / Rainforest API)
- [ ] OpenAI embedding API entegrasyonu
...
| **Stack** | React + tRPC + Drizzle + PostgreSQL | Type-safe, modern, skill desteği var |
