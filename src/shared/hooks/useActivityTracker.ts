...
export function useActivityTracker(prospectId: number) {
  ...
  // Return stable reference so consumers can use it in dependency arrays safely
  return useMemo(() => {
    const trackEvent = (eventType: string, eventData?: Record<string, unknown>, scoreIncrement = 0) => {
      ...
      setInterestScore((prev) => {
        ...
        // Fire mutation asynchronously; don't block state update
  ...
  }, [prospectId, recordActivity, interestScore]);
}
